...
if($arrDados["acao"]=="insert")
{
  $data	= json_decode($arrDados['data']);	
	...
	$numero	= mysql_escape_string($data->{'numero'}); 	
	$flag_tipo = mysql_escape_string($data->{'flag_tipo'}); 	 			

	$strSQL  = "INSERT INTO conta ";
	$strSQL .= " (conta_id, numero, flag_tipo) "; 
	$strSQL .= "VALUES ('".$conta_id."', '".$numero."', '".flag_tipo."')";
...
else if($arrDados["acao"]=="update")
{
	...
	$numero 		= mysql_escape_string($arrDados['numero']); 	
...
else 
{
        ...
        $strSQL = "SELECT id, conta_id, numero, flag_tipo FROM conta ORDER BY ".mysql_real_escape_string($order);
