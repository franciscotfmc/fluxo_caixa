...
if($arrDados["acao"]=="insert")
{
  $data	= json_decode($arrDados['data']);	
	...
	$strSQL = "INSERT INTO usuario (name, email, password) VALUES ('".$name."','".$email."','".$password."')";
...
else 
{
        ...
        $strSQL = "SELECT id, name, email, password FROM usuario ORDER BY ".mysql_real_escape_string($order);
